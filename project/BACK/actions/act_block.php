...
if ($action === 'add_block_single') {
    ...
    $design_per_course = isset($_POST['design_per_course']) ? (int)$_POST['design_per_course'] : 0;
    ...
    $line_list = implode(',', array_map(function($course_no) use ($design_per_course) {
        return $design_per_course > 0 ? ($course_no . ':' . $design_per_course) : (string)$course_no;
    }, $new_courses));
...
if ($action === 'add_block_bulk') {
    ...
    foreach ($lines as $line) {
        ...
        if (count($parts) >= 2) {
            $b_name = trim($parts[0]);
            $c_no = (int)$parts[1];
            $design_count = isset($parts[2]) ? (int)$parts[2] : 0;
            if ($b_name && $c_no > 0) {
                ...
                $design_map[$b_name][$c_no] = max(0, $design_count);
            }
        }
    }
    ...
    foreach ($block_map as $b_name => $courses) {
        ...
        $line_items = [];
        foreach ($unique_courses as $course_no) {
            $design_count = (int)($design_map[$b_name][$course_no] ?? 0);
            $line_items[] = $design_count > 0 ? ($course_no . ':' . $design_count) : (string)$course_no;
        }
        $line_list = implode(',', $line_items);
